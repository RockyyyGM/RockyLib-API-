package dev.rocky.rockylib.event;


import com.google.common.collect.MapMaker;
import dev.rocky.rockylib.api.event.v1.ArrayBackedEvent;
    ...
    private static <T> T buildEmptyInvoker(Class<T> handlerClass, Function<T[], T> invokerSetup) {
        ...
        for (Method m : handlerClass.getMethods()) {
            if ((m.getModifiers() & (Modifier.STRICT | Modifier.PRIVATE)) == 0) {
                if (funcIfMethod != null) {
                    throw new IllegalStateException("Multiple virtual methods in " + handlerClass + "; cannot build empty "
                            + "invoker!");
                }
        ...
        try {
            ...
            if (type.returnType() != void.class) {
                // determine default return value by invoking invokerSetup.apply(T[0]) with all-jvm-default args (null for
                // refs, false for boolean, etc.)
